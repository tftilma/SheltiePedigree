...
public class GenStatistics extends XmlWriterService {
	...
	private final static int TOP_N_OFFSPRINGSIZE = 100;
	private final static int TOP_N_KENNELSIZE = 100;
	
	
	public void generate()
	throws Exception {
	    Document statisticsDoc = genStatisticsDoc();
	    Transform.transform(statisticsDoc, "xml//statistics.xslt", STATISTICS_FILE_NAME);
	}
	...
	private Document genStatisticsDoc()
	  throws TransformerException, ParserConfigurationException
	  {
	    ...
	    Collection animals = maps.getAllAnimals();
	    ...
	    for (int idx=0; idx < TOP_N_OFFSPRINGSIZE; idx++)
	    {
	      ...
	      mostOffspringNode.appendChild(createTextNode("topperid", animalsWithMostOffspring[idx].getId()));
	    ...
	    for (int idx=0; idx < TOP_N_KENNELSIZE; idx++)
	    {
	...
	private void genLines(Node rootNode, LinesFamMap percMaps) {
		...
		genPerc(linesNode, percMaps, "???", "percLine");
	...
	private void genColors(Node rootNode, LinesFamMap percMaps) {
		...
		genPerc(linesNode, percMaps, "???", "percColor");
	...
	private void genFams(Node rootNode, LinesFamMap percMaps) {
		...
		genPerc(linesNode, percMaps, "???", "percFam");
	...
	private Animal []calcTopMostOffspring(Collection animals)
	  {
	    Animal []animalsWithMostOffspring = new Animal[2*TOP_N_OFFSPRINGSIZE];// 2* so some extra space
	    ...
	    int i=0;
	    for (Iterator animalIter = animals.iterator();animalIter.hasNext();)
	    {
	      //String animalId = (String) animalIter.next();
	      Animal animal = (Animal) animalIter.next();
	      	//maps.findAnimalById(animalId); 
	      	//(Animal) animalMap.get(animalId);
	      if (animal.getNChildren() >= 14)
	      {
	    ...
	    Comparator comp = new AnimalsWithMostOffspringComparator();
	    Arrays.sort(animalsWithMostOffspring, comp);
	    return animalsWithMostOffspring;
	  }

	  // TODO
	  private Kennel []calcTopLargestKennels(Collection kennels)
	  {
	    Kennel []largestKennels = new Kennel[2*TOP_N_KENNELSIZE];
	    // Filling of empty animals
	    ...
	    for (Iterator kennelIter = kennels.iterator(); kennelIter.hasNext();)
	    {
	      ...
	      if (!"empty".equals(kennel.getKennelId()) && kennel.getNAnimals() >= 23)
	      {
	    ...
	    Comparator comp = new KennelsWithMostAnimalsComparator();

...
public class RegMap {
	...
	public void addAnimal(Animal animal) {
		...
		if (reg != null) {
			try {
				if (reg.getRegYear() != null && reg.getRegMonth() != null) {
					int year = Integer.parseInt(reg.getRegYear());
					int month = Integer.parseInt(reg.getRegMonth());
					addRegToAnimal(year, month, animal);
				}
			} catch (RuntimeException e) {
				System.out.println("ERROR: " + animal.getAnimalid());
				throw e;
			}

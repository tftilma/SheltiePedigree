...
public class GenAnimalsPerKennelService extends XmlWriterService {
	...
	public void generate() throws TransformerException,
		...
		List<Kennel> kennelList = new ArrayList<Kennel>();
		kennelList.addAll(kennels);
		Collections.sort(kennelList);
		for (Iterator<Kennel> kennelIdIter = kennelList.iterator(); kennelIdIter.hasNext();) {
			Kennel kennel = kennelIdIter.next();
			...
			Transform.transform(kennelWithAnimalsDoc,
					"src//main//resources//xml//knlanimals2html.xslt", kennelFileName);
		}
	}
	...
	private Node generateKennelWithAnimals(String kennelId)
		...
		List<Animal> animals = maps.findAnimalsByKennelId(kennelId);

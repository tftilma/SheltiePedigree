...
public class GenChampions extends XmlWriterService {
	...
	public void generate() throws Exception {
		position = 1;
		String country = "NL";
		List<Animal> sortedAnimals = maps.getNlChList().getChList();
	...
	private void genChampionLists(String country, List<Animal> sortedAnimals,
	...
	throws TransformerException, ParserConfigurationException {
		...
		Document statisticsDoc = genChampionListDoc(country, sortedAnimals,
				startyear, endyear);
		Transform.transform(statisticsDoc, "src/main/resources/xml/champions.xslt",
				championsFileName);
	}
	
	private Document genChampionListDoc(String country, List<Animal> sortedAnimals,
			int startyear, int endyear) 
	throws TransformerException, ParserConfigurationException {
		...
		Animal animal = null;
		for (Iterator<Animal> iter = sortedAnimals.iterator(); iter.hasNext();) {
			prevAnimal = animal;
			animal = iter.next();
			if (animal == null) {
			...
			if (prevAnimal != null) {
				if ("Eng".equals(country)) {
					prevAnimal.setNextEngChId(animal.getAnimalid());
					animal.setPrevEngChId(prevAnimal.getAnimalid());
				} else if ("NL".equals(country)) {
					prevAnimal.setNextNlChId(animal.getAnimalid());
					animal.setPrevNlChId(prevAnimal.getAnimalid());
				}
			}
